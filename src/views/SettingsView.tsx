          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-semibold text-white">
                Sincronização em Nuvem (Supabase & Multi-Dispositivo)
              </h2>
            ...
            <p className="text-xs text-slate-400 mt-1 max-w-xl leading-relaxed">
              O aplicativo opera 100% offline no seu computador. Ao conectar com o Supabase, seus dados são salvos na nuvem e você pode acessá-los de outros computadores fazendo login.
            </p>
          </div>

          <button
            onClick={() => setCloudAuthModalOpen(true)}
            className="px-3.5 py-2 bg-brand-600 hover:bg-brand-500 text-white rounded-xl text-xs font-semibold transition-all shadow-sm shrink-0"
          >
            {cloudSession?.userId ? 'Gerenciar Conta' : 'Conectar ao Supabase'}
          </button>
        </div>

        {cloudSession?.userId ? (
          <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-850 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <span className="font-semibold text-white">Conectado como {cloudSession.email}</span>
              </div>
              <div className="flex items-center gap-1.5 text-[11px] text-emerald-400 font-medium">
                <Shield className="w-3.5 h-3.5 text-emerald-400" />
                <span>Nuvem Ativa • Backup Criptografado</span>
              </div>
              <span className="text-[11px] text-slate-400 block">
                {cloudSession.lastSyncAt
                  ? `Última sincronização: ${new Date(cloudSession.lastSyncAt).toLocaleString('pt-BR')}`
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-750 text-slate-200 rounded-lg text-xs font-semibold transition-colors"
              >
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/40 border border-slate-850 text-xs text-slate-400 flex items-center justify-between">
            <span>Status atual: <strong>Modo 100% Local / Offline</strong> (sem login em nuvem).</span>
          </div>
        )}
      </div>
